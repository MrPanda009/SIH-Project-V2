...
export default function AuthLogin({ onLoginSuccess, onSwitchToSignup }: AuthLoginProps) {
  ...
  // A constant OTP for bypassing verification during development
  ...
  const handleStep1Submit = async (e: React.FormEvent) => {
    ...
    // In a real implementation, this would send OTP to email
    // For demo purposes, we'll just move to step 2
    console.log(`Magic OTP for ${formData.email} is: ${MAGIC_OTP}`) // Log for easy testing
    setStep(2)
    setError('')
  }
  ...
  const handleStep2Submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. OTP Validation
      if (formData.otp !== MAGIC_OTP) {
        throw new Error('Invalid OTP. Please use the magic OTP for testing.')
      }
  ...
  const handleForgotPasswordStep1 = async (e: React.FormEvent) => {
    ...
    try {
      // Call API to send OTP to registered mobile and email
      // await authAPI.forgotPassword(forgotPasswordData.aadhaar)
      console.log(`Forgot Password: Magic OTP for Aadhaar ${forgotPasswordData.aadhaar} is: ${MAGIC_OTP}`)
  ...
  const handleForgotPasswordStep2 = (e: React.FormEvent) => {
    ...
    if (forgotPasswordData.otp === MAGIC_OTP) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-orange-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-xl">Civic Issue Reporting</CardTitle>
          <CardDescription>
